...
def do_search(
    query: str, source: str = "all", limit: int = 10, console: Optional[Console] = None
) -> None:
...
def do_install(
    ...
    if not bundle:
        c.print(
            f"[bold red]Error:[/] Could not fetch '{identifier}' from any source.\n"
        )
        return
...
def do_list(source_filter: str = "all", console: Optional[Console] = None) -> None:
    ...
    for skill in sorted(all_skills, key=lambda s: (s.get("category") or "", s["name"])):
        ...
        table.add_row(
            name, category, source_display, f"[{trust_style}]{trust_label}[/]"
        )
...
def skills_command(args) -> None:
    ...
    if action == "browse":
        ...
        _console.print(
            "Usage: hermes skills [browse|search|install|inspect|list|check|update|audit|uninstall|publish|snapshot|tap]\n"
        )
...
def handle_skills_slash(cmd: str, console: Optional[Console] = None) -> None:
    ...
    if action == "browse":
        ...
        if not args:
            c.print(
                "[bold red]Usage:[/] /skills search <query> [--source skills-sh|well-known|github|official] [--limit N]\n"
            )
            return
        source = "all"
        limit = 10
        ...
        if not args:
            c.print(
                "[bold red]Usage:[/] /skills install <identifier> [--category <cat>] [--force] [--now]\n"
            )
            return
        identifier = args[0]
        category = ""
