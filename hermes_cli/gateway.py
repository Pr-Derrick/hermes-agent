...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    venv_bin = (
        str(detected_venv / "bin")
        if detected_venv
        else str(PROJECT_ROOT / "venv" / "bin")
    )
...
def systemd_install(
    force: bool = False, system: bool = False, run_as_user: str | None = None
):
    if system:
    ...
    if unit_path.exists() and not force:
        if not systemd_unit_is_current(system=system):
            ...
            subprocess.run(
                _systemctl_cmd(system) + ["enable", get_service_name()],
                check=True,
                timeout=30,
            )
            print(
    ...
    subprocess.run(_systemctl_cmd(system) + ["daemon-reload"], check=True, timeout=30)
    subprocess.run(
        _systemctl_cmd(system) + ["enable", get_service_name()], check=True, timeout=30
    )

    print()
    print(
        f"✓ {_service_scope_label(system).capitalize()} service installed and enabled!"
    )
    print()
    print("Next steps:")
    print(
...
def systemd_uninstall(system: bool = False):
    ...
    subprocess.run(
        _systemctl_cmd(system) + ["stop", get_service_name()], check=False, timeout=90
    )
    subprocess.run(
        _systemctl_cmd(system) + ["disable", get_service_name()],
        check=False,
        timeout=30,
    )
...
def systemd_start(system: bool = False):
    ...
    refresh_systemd_unit_if_needed(system=system)
    subprocess.run(
        _systemctl_cmd(system) + ["start", get_service_name()], check=True, timeout=30
    )
...
def systemd_stop(system: bool = False):
    ...
    subprocess.run(
        _systemctl_cmd(system) + ["stop", get_service_name()], check=True, timeout=90
    )
...
def systemd_restart(system: bool = False):
    ...
    if pid is not None and _request_gateway_self_restart(pid):
        ...
        return
    subprocess.run(
        _systemctl_cmd(system) + ["reload-or-restart", get_service_name()],
        check=True,
        timeout=90,
    )
...
def _setup_weixin():
    ...
    if group_idx == 0:
        ...
        allow_groups = prompt(
            "  Allowed group chat IDs (comma-separated)", "", password=False
        ).replace(" ", "")
    ...
    if any_configured:
        ...
        if service_running:
            ...
            if supports_systemd_services() or is_macos():
                ...
                if prompt_yes_no(
                    f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)",
                    True,
                ):
                    try:
                        ...
                        if supports_systemd_services():
                            installed_scope, did_install = (
                                install_linux_gateway_from_setup(force=False)
                            )
                        else:
                            launchd_install(force=False)
                            did_install = True
                        print()
                        if did_install and prompt_yes_no(
