...
def _send_media_via_adapter(
    adapter, chat_id: str, media_files: list, metadata: dict | None, loop, job: dict
) -> None:
        ...
        try:
            ...
            if adapter_ok and media_files:
                _send_media_via_adapter(
                    runtime_adapter, chat_id, media_files, send_metadata, loop, job
                )
    ...
    if script_path:
        ...
        if success:
            if script_output:
                ...
                prompt = (
                    f"[Script ran successfully but produced no output.]\n\n{prompt}"
                )
        else:
            prompt = (
                "## Script Error\n"
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    try:
        ...
        if delivery_target:
            os.environ["HERMES_CRON_AUTO_DELIVER_PLATFORM"] = delivery_target[
                "platform"
            ]
            os.environ["HERMES_CRON_AUTO_DELIVER_CHAT_ID"] = str(
                delivery_target["chat_id"]
            )
            if delivery_target.get("thread_id") is not None:
                os.environ["HERMES_CRON_AUTO_DELIVER_THREAD_ID"] = str(
        ...
        runtime_provider = (
            str(turn_route["runtime"].get("provider") or "").strip().lower()
        )
        if runtime_provider:
            try:
