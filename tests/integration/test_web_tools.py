    ...
    async def test_web_extract(self, urls: List[str] = None):
        ...
        if urls:
            try:
                ...
                if self.verbose:
                    print(
                        f"  Calling web_extract_tool(urls={test_urls}, format='markdown', use_llm_processing=False)"
                    )

                result = await web_extract_tool(
                    test_urls, format="markdown", use_llm_processing=False
                )
    ...
    async def test_web_crawl(self):
        ...
        for url, instructions, expected_min_pages in test_sites:
            try:
                ...
                if self.verbose:
                    print(
                        f"  Calling web_crawl_tool(url='{url}', instructions={instructions}, use_llm_processing=False)"
                    )
                ...
                try:
                    ...
                    self.log_result(
                        f"Crawl: {url}", "failed", f"Invalid JSON response: {e}"
                    )
                    if self.verbose:
                        print(f"    Raw response (first 500 chars): {result[:500]}...")
                    continue
                ...
                if "error" in data:
                    self.log_result(
                        f"Crawl: {url}", "failed", f"API error: {data['error']}"
                    )
                    continue
                ...
                if not results:
                    self.log_result(
                        f"Crawl: {url}", "failed", "No pages in results array"
                    )
                    if self.verbose:
                        print(
