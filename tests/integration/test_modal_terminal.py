...
def test_modal_requirements():
    ...
    print(
        f"  ~/.modal.toml file: {'✅ Exists' if modal_toml.exists() else '❌ Not found'}"
    )
...
def test_simple_command():
    ...
    success = result_json.get(
        "exit_code"
    ) == 0 and "Hello from Modal!" in result_json.get("output", "")
...
def test_python_execution():
    ...
    success = result_json.get("exit_code") == 0 and "Python" in result_json.get(
        "output", ""
    )
    print(f"\nTest: {'✅ Passed' if success else '❌ Failed'}")
...
def test_pip_install():
    ...
    print(
        "Executing: pip install --break-system-packages cowsay && python3 -c \"import cowsay; cowsay.cow('Modal works!')\""
    )
    ...
    print(
        f"  Output (last 500 chars): ...{output[-500:] if len(output) > 500 else output}"
    )
    print(f"  Exit code: {result_json.get('exit_code')}")
    print(f"  Error: {result_json.get('error')}")

    success = result_json.get("exit_code") == 0 and "Modal works!" in result_json.get(
        "output", ""
    )
    print(f"\nTest: {'✅ Passed' if success else '❌ Failed'}")
...
def test_filesystem_persistence():
    ...
    result1 = terminal_tool(
        "echo 'persistence test' > /tmp/modal_test.txt", task_id=test_task_id
    )
    ...
    success = (
        result1_json.get("exit_code") == 0
        and result2_json.get("exit_code") == 0
        and "persistence test" in result2_json.get("output", "")
    )
    print(f"\nTest: {'✅ Passed' if success else '❌ Failed'}")
...
def test_environment_isolation():
    ...
    result2 = terminal_tool(
        "cat /tmp/isolated.txt 2>&1 || echo 'FILE_NOT_FOUND'", task_id=task2
    )
    result2_json = json.loads(result2)
    ...
    isolated = (
        "task1 data" not in output
        or "FILE_NOT_FOUND" in output
