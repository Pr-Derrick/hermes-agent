...
def check_tinker_atropos():
    ...
    if not tinker_path.exists():
        return (
            False,
            "tinker-atropos submodule not found. Run: git submodule update --init",
        )
        ...
        try:
            ...
            for env in envs:
                ...
                if env.get("description"):
                    desc = (
                        env["description"][:100] + "..."
                        if len(env.get("description", "")) > 100
                        else env.get("description", "")
                    )
            ...
            try:
                ...
                if user_input.lower() == "status":
                    ...
                    if isinstance(runs, list) and runs:
                        ...
                        for run in runs:
                            print(
                                f"  - {run['run_id']}: {run['environment']} ({run['status']})"
                            )
                    else:
                        print("\nNo active runs.")
                    continue
