...
class InsightsEngine:
    ...
    def format_terminal(self, report: Dict) -> str:
        ...
        cache_total = o.get("total_cache_read_tokens", 0) + o.get(
            "total_cache_write_tokens", 0
        )
        if cache_total > 0:
            lines.append(
                f"  Cache read:        {o['total_cache_read_tokens']:<12,}  Cache write:     {o['total_cache_write_tokens']:,}"
            )
        cost_str = f"${o['estimated_cost']:.2f}"
        if o.get("models_without_pricing"):
            cost_str += " *"
        lines.append(
            f"  Total tokens:      {o['total_tokens']:<12,}  Est. cost:       {cost_str}"
        )
        if o["total_hours"] > 0:
            lines.append(
        ...
        if report["models"]:
            ...
            for m in report["models"]:
                ...
                if m.get("has_pricing"):
                    ...
                    cost_cell = "     N/A"
                lines.append(
                    f"  {model_name:<30} {m['sessions']:>8} {m['total_tokens']:>12,} {cost_cell}"
                )
            if o.get("models_without_pricing"):
    ...
    def format_gateway(self, report: Dict) -> str:
        ...
        if cache_total > 0:
            lines.append(
                f"**Tokens:** {o['total_tokens']:,} (in: {o['total_input_tokens']:,} / out: {o['total_output_tokens']:,} / cache: {cache_total:,})"
            )
        else:
            lines.append(
