...
class RunState:
    ...
    config: Dict[str, Any]
    status: str = (
        "pending"  # pending, starting, running, stopping, stopped, completed, failed
    )
    error_message: str = ""
    wandb_project: str = ""
    wandb_run_name: str = ""
                        ...
                        if base_name == "BaseEnv":
                            ...
                            environments.append(
                                EnvironmentInfo(
                                    ...
                                    or f"Environment from {py_file.name}",
    ...
    try:
        ...
        for name, obj in vars(module).items():
            if isinstance(obj, type) and name != "BaseEnv":
                if hasattr(obj, "config_init") and callable(
                    getattr(obj, "config_init")
                ):
                    env_class = obj
                    break
        ...
        try:
            ...
            logger.info(
                "config_init failed (%s), using BaseEnvConfig defaults", config_error
            )
            try:
...
async def _spawn_training_run(run_state: RunState, config_path: Path):
    ...
    try:
        ...
        logger.info(
            "[%s] Starting Tinker trainer: launch_training.py --config %s",
            run_id,
            config_path,
        )
        ...
        logger.info(
            "[%s] Waiting 90 more seconds before starting environment...", run_id
        )
    ...
    while run_state.status == "running":
        ...
        if run_state.env_process and run_state.env_process.poll() is not None:
            ...
            if exit_code == 0:
                ...
                run_state.error_message = (
                    f"Environment process exited with code {exit_code}"
                )
            _stop_training_run(run_state)
            break
        ...
        if run_state.trainer_process and run_state.trainer_process.poll() is not None:
            ...
            if exit_code == 0:
                ...
                run_state.status = "failed"
                run_state.error_message = (
                    f"Trainer process exited with code {exit_code}"
                )
            _stop_training_run(run_state)
            break
...
async def rl_check_status(run_id: str) -> str:
    ...
    processes = {
        ...
        "trainer": run_state.trainer_process.poll()
...
async def rl_test_inference(
    ...
    for model_info in test_models:
        ...
        cmd = [
            ...
            str(group_size),
            "--env.use_wandb",
            "true",  # Enable wandb for test tracking
            "--env.wandb_name",
            wandb_run_name,
            ...
            "--openai.health_check",
            "false",  # OpenRouter doesn't have health endpoint
        ]
        ...
        if model_results["total_completions"] > 0:
            model_results["accuracy"] = round(
                model_results["correct_completions"]
                / model_results["total_completions"],
                3,
            )
        else:
        ...
        if model_results["steps_tested"] > 0:
            steps_with_correct = sum(
                1 for s in model_results["steps"] if s.get("correct", 0) > 0
            )
    ...
    # Overall summary
    working_models = [
        m for m in results["models_tested"] if m.get("steps_tested", 0) > 0
    ]
...
registry.register(
    ...
    handler=lambda args, **kw: rl_edit_config(
