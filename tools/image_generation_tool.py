...
def _validate_parameters(
    ...
    if isinstance(image_size, str):
        ...
        if not isinstance(image_size["width"], int) or not isinstance(
            image_size["height"], int
        ):
        ...
        raise ValueError(
            "image_size must be either a preset string or a dict with width/height"
        )
    ...
    if (
        ...
        or guidance_scale < 0.1
        or guidance_scale > 20.0
    ):
    ...
    if acceleration not in VALID_ACCELERATION_MODES:
        raise ValueError(
            f"Invalid acceleration '{acceleration}'. Must be one of: {VALID_ACCELERATION_MODES}"
        )
...
def image_generate_tool(
    ...
    aspect_ratio_lower = (
        aspect_ratio.lower().strip() if aspect_ratio else DEFAULT_ASPECT_RATIO
    )
    ...
    try:
        logger.info(
            "Generating %s image(s) with FLUX 2 Pro: %s", num_images, prompt[:80]
        )
        ...
        upscaled_count = sum(
            1 for img in formatted_images if img.get("upscaled", False)
        )
        logger.info(
...
if __name__ == "__main__":
    ...
    if _debug.active:
        ...
        print(
            f"   Debug logs will be saved to: ./logs/image_tools_debug_{_debug.session_id}.json"
        )
    else:
