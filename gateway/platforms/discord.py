...
class DiscordAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            intents.members = any(
                not entry.isdigit() for entry in self._allowed_user_ids
            )
        ...
        try:
            ...
            for discord_name, description, cmd_key in skill_entries:
                ...
                def _make_skill_handler(_key: str):
                    async def _skill_slash(
                        interaction: discord.Interaction, args: str = ""
                    ):
                        await self._run_simple_slash(
                            interaction, f"{_key} {args}".strip()
                        )
            ...
            logger.warning(
                "[%s] Failed to register skill slash commands: %s", self.name, exc
            )
